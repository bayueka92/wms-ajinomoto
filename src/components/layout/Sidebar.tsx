...
const SidebarItem: React.FC<SidebarItemProps> = ({ to, icon, label, isActive }) => {
  return (
    <Link
      to={to}
      className={`flex items-center px-3 py-2 my-1 text-sm font-medium rounded-md group transition-colors ${isActive
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  ...
  return (
    <aside
      className={`bg-white shadow-sm h-screen z-20 transition-all duration-300 ${isOpen ? 'w-64' : 'w-0 -translate-x-full md:w-16 md:translate-x-0'
        } border-r border-ajinomoto-gray-200`}
    >
      <div className="h-full flex flex-col">
        <div className={`flex items-center justify-center h-16 border-b border-ajinomoto-gray-200 ${isOpen ? 'px-4' : 'px-2'
          }`}>
          {isOpen ? (
            <img
